...
def R_X_Y_obg(iSelectedObject): # получение координат, указатель на объект и указатенль на вид
    ...
    elif Type_obj == 32 and Ellipse and iSelectedObject.Style in iteration(Style): # обработка объектов по типу объекта и стилю (32 - эллипс, см. DrawingObjectTypeEnum)
    ...
    elif Type_obj == 34 and Arc_ellipse and iSelectedObject.Style in iteration(Style): # обработка объектов по типу объекта и стилю (34 - дуга эллипса, см. DrawingObjectTypeEnum)
...
def Conditional_sign(x, y, R, obj, iView): # простановка условных знаков
    ...
    iInsertionDefinition = iInsertionsManager.AddDefinition(0, Conditional_sign_name, Conditional_sign_name) # cоздать новое описание для фрагмента или вида
