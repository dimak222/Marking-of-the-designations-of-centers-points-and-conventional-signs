...
Conditional_sign_list_name_v16 = {1:"Shapes_and_signs.lfr||Условные знаки|Условный знак 1_1", 2:"Shapes_and_signs.lfr||Условные знаки|Условный знак 1_2", 3:"Shapes_and_signs.lfr||Условные знаки|Условный знак 1_3", 4:"Shapes_and_signs.lfr||Условные знаки|Условный знак 1_4",
                                  ...
                                  # имя условного знака для КОМПАС v16 (для использования переименуйте в "Conditional_sign_list_name"), (имя библиотеки, имя знака (см. название в "Библиотека фигур и усовных знаков"))

Conditional_sign_names = "1-10" # номер условного знака из списка, "1-10" - знаки с 1-го по 10-й; "1-5,7,9" - знаки с 1-го по 5-й, 7-ой, 9-й; "1" - только первый

Conditional_sign_size_dependence = True # ставить ли разные условные знаки в зависимости от размера выделенных объектов (True - да, False - нет)
Conditional_sign_sort = False # сортировка по убыванию размеров выделенных объектов (True - да, False - нет)
Conditional_sign_number_identical_objects = 2 # с какого количества одинаковых объектов ставить условные знаки (с n включительно)
...
def Creating(list_Obg): # создание центров и точек
    ...
    def Create(OBG, Repeat, list1, list2): # создать ОЦ/Т

        score = 0 # количество обработаных объектов

        iDrawingGroups = iKompasDocument2D1.DrawingGroups # интерфейс коллекции групп (именованных, рабочих временных и постоян­ных)
        iDrawingGroup = iDrawingGroups.Add(True, "Группа ОЦ/Т") # интерфейс группы (постоянной, временной, именованной)
        iDrawingGroup.Open() # открыть группу для редактирования
        ...
        for obg in (list1): # строим обозначения центров/точек во все объекты со списка
            OBG(obg[1], obg[2], obg[0], obg[3], obg[4]) # простановка обозначения центров/точек (радиус/размер, положение X, положение Y, объект, вид)
            score += 1 # подсчёт количества обработаных объектов

        iDrawingGroup.Close() # закрыть группу для редактирования
        iDrawingGroup.Store() # вставить временную группу в документ

        return score # возвращаем количество обработаных объектов

    def Create_Conditional_sign(OBG, Repeat, list1, list2): # создать условный знак (функция простановки УЗ, простановка УЗ на уже выделеные УЗ, список объектов, список УЗ)

        score = 0 # количество обработаных объектов

##        iDrawingGroups = iKompasDocument2D1.DrawingGroups # интерфейс коллекции групп (именованных, рабочих временных и постоян­ных)
##        iDrawingGroup = iDrawingGroups.Add(True, "Группа УЗ") # интерфейс группы (постоянной, временной, именованной)
##        iDrawingGroup.Open() # открыть группу для редактирования
        ...
        if Repeat == False: # если повторные значения не нужны
            ...
            if len(list1) == 0: # если УЗ уже стоят
                score_Conditional_sign = "Условные знаки уже проставлены!" # текст сообщения
                return score_Conditional_sign # возвращаем текст последнего сообщения

        Conditional_sign_names_list = iteration(Conditional_sign_names) # обработка перечисленых цифр

        if Conditional_sign_size_dependence: # ставить ли условные знаки в зависимости от размера выделенных объектов (True - да, False - нет)
            ...
            for obg in list1: # обрабатываем каждый объект
                List_Temp.append(obg[0]) # добавляем запись размеров всех выделенных объектов в список

            List_Temp2 = [] # временый список для расчёта
            obg_size_old = "" # размер старого выделеного объекта
            n = 1 # номер УЗ
            Conditional_sign_names_dict = {} # словарь для определения номера УЗ от размера выделеного объекта

            for obg in list1: # обрабатываем каждый объект
                obg_size = obg[0] # размер выделеного объекта
                if List_Temp.count(obg_size) >= Conditional_sign_number_identical_objects: # если количество объектов больше заданного числа
                    List_Temp2.append(obg) # добавляем запись размеров выделенных объектов в список

                if obg_size_old == obg_size or obg_size_old == "": # если размер старый размер равен новому или пуст
                    Conditional_sign_names_dict[obg_size] = n # добавляем размер и номер УЗ в словарь
                    obg_size_old = obg_size # записываем размер как старый
                else: # если размер новый
                    n += 1 # следующий номер УЗ
                    Conditional_sign_names_dict[obg_size] = n # добавляем размер и номер УЗ в словарь
                    obg_size_old = obg_size # записываем размер как старый

            list1 = List_Temp2 # список только повторяющихся объектов

            if len(list1) == 0: # если нет одинаковых объектов
                score_Conditional_sign = "Одинаковые объекты не обнаружены!" # текст сообщения
                return score_Conditional_sign # возвращаем текст последнего сообщения

        for obg in list1: # строим обозначения во все объекты со списка

            obg_size = obg[0] # размер выделеного объекта

            if Conditional_sign_size_dependence: # ставить ли условные знаки в зависимости от размера выделенных объектов (True - да, False - нет)
                Conditional_sign_name = Conditional_sign_names_dict[obg_size] # номер УЗ
            else: # ставить одинаковые условные знаки на все объекты
                Conditional_sign_name = int(Conditional_sign_names_list[0]) # # номер УЗ (первое значение со списка)

            if Conditional_sign_name > len(Conditional_sign_names_list): # проверка на количество УЗ
                Message("Закончились УЗ введёные в \"Conditional_sign_names\"!\nСоздайте в библиотеке новые и введите в список \"Conditional_sign_list_name\".", counter = 8) # сообщение, поверх всех окон с автоматическим закрытием (текст, время закрытия)
                break # заканчиваем цикл

            OBG(obg[1], obg[2], obg_size, obg[3], obg[4], Conditional_sign_name) # простановка центров окружностей (радиус/размер, положение X, положение Y, объект, вид)
            score += 1 # подсчёт количества обработаных объектов
...
##        iDrawingGroup.Store() # вставить временную группу в документ
    ...
    if Conditional_sign_var: # ставить ли условные обозначения
        ...
        if Conditional_sign_on_point: # если включено ставить ли условные знаки на точки
            list_Obg = list_Obg + list_Point # добавляем в список объектов точки
        score_Conditional_sign = Create_Conditional_sign(Conditional_sign, Conditional_sign_repeat, list_Obg, list_Conditional_sign) # создать условный знак (функция простановки УЗ, простановка УЗ на уже выделеные УЗ, список объектов, список УЗ)
...
def Conditional_sign(x, y, R, obj, iView, Conditional_sign_name): # простановка условных знаков
...
def End_msg(score_Centre, score_Point, score_Conditional_sign): # вывод сообщения о количестве обработаных файлов
    ...
    if score_Point != 0: # если проставлены точки
    ...
    if isinstance(score_Conditional_sign, int): # если количество УЗ целое число
        if score_Conditional_sign != 0: # если проставлены точки
        ...
        else: # если нет проставленых УЗ
            text_Conditional_sign = "Условные знаки уже проставлены!" # текст сообщения

    else: # если значение текстовое
        text_Conditional_sign = score_Conditional_sign # текст сообщения
