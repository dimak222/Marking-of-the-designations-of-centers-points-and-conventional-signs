...
Style = range(1, 26) # Стиль линий объектов, которые нужно обрабатывать. Перечислить номера через запятую, напимер: style = (2,); style = (1, 3, 5, 6) или style = range(1, 26) (все стили)
...
def R_X_Y_obg(iSelectedObject): # получение координат и указатель на объект
    ...
    if Type_obj in (2, 3, 36): # Обработка объектов по типу объекта (2 - окружность; 3 - дуга; 36 - многоугольник, см. DrawingObjectTypeEnum)
        ...
        BD.append([R, X, Y, iSelectedObject, iView]) # добавляем запись координат в список
...
def list_processing(): # обработка списка (удаление лишнего и сортировка отв. от большего к меньшему)
    ...
    if len(BD) >= 1: # если в списке больше 1 объекта
        ...
        BD.sort(key = lambda x: x[0]) # сортируем его по увеличению радиусов
    ...
    for n in range(len(BD)): # перебор значений из диапазона количества объектов
        ...
        while m < len(BD): # цикл проверки последующих объектов
            ...
            if delta != 0: # если delta не ноль, продолжать обработку
                if abs(BD[n][1] - BD[m][1]) <=  delta and abs(BD[n][2] - BD[m][2]) <=  delta: # если координата X и Y объекта n совпадает с координатами X и Y объекта m
...
def Creating(): # создание центров и точек
    ...
    for k in range(len(BD)): # строим обозначения центров во все объекты со списка
        ...
        Point(BD[k][1], BD[k][2], BD[k][3], BD[k][4]) # простановка точек в окружности (положение X, положение Y, объект, вид)
    ...
    Text(score) # вывод сообщения о количестве обработаных файлов
...
def Text(score): # вывод сообщения о количестве обработаных файлов

    if str(score)[-1] in ["0","5","6","7","8","9"]: # если значение заканчиваеться на "0","5","6","7","8","9"
        text = "обозначений" # окончание в зависимости от значения
    elif str(score)[-1] == "1": # если значение заканчиваеться на "1"
        text = "обозначение" # окончание в зависимости от значения
    else: # если другое
        text = "обозначения" # окончание в зависимости от значения

    Kompas_message("Проставлено: " + str(score) + " " + text) # сообщение в окне компаса если он открыт
