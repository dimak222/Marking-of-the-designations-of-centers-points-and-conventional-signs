...
def Creating(): # создание центров и точек
    ...
    def list_processing(): # обработка списка (удаление лишнего и сортировка отв. от большего к меньшему)
        ...
        if len(list_Obg) >= 1: # если в списке больше 1 объекта
            ...
            list_Obg.sort(key = lambda x: x[0]) # сортируем его по увеличению размеров (по первому значению)
    ...
    if Centre_var: # ставить ли обозначения центров
        for k in range(len(list_Obg)): # строим обозначения центров во все объекты со списка
            Centre(list_Obg[k][1], list_Obg[k][2], list_Obg[k][0], list_Obg[k][3], list_Obg[k][4]) # простановка центров окружностей (радиус/размер, положение X, положение Y, объект, вид)
            score += 1 # подсчёт количества обработаных объектов

    if Point_var: # ставить ли точки
        for k in range(len(list_Obg)): # строим обозначения центров во все объекты со списка
            Point(list_Obg[k][1], list_Obg[k][2], list_Obg[k][3], list_Obg[k][4]) # простановка точек в окружности (положение X, положение Y, объект, вид)
            score += 1 # подсчёт количества обработаных объектов

    iDocument2D.ksEndGroup # завершить создание группы
    iDocument2D.ksStoreTmpGroup(iGroup) # вставляем группу в чертёж

    End_msg(score) # вывод сообщения о количестве обработаных файлов
...
def End_msg(score): # вывод сообщения о количестве обработаных файлов
