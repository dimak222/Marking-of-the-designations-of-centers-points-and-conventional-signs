...
def iSelectedObjects_processing(iSelectedObject): # обработка выбраных объектов
    ...
    if iSelectedObject.DrawingObjectType == 123: # если вид (тип графических объектов (123 - вид., см DrawingObjectTypeEnum))
        View_processing() # обработка всего вида
    ...
    for k in range(len(BD)): # строим обозначения центров во все объекты со списка
        Centre(BD[k][1], BD[k][2], BD[k][0], BD[k][3], BD[k][4]) # простановка центров окружностей (радиус/размер, положение X, положение Y, номер вида)
        Point(BD[k][1], BD[k][2], BD[k][3], BD[k][4]) # простановка точек в окружности (радиус/размер, положение X, положение Y, номер вида)
    ...
    iDrawingObject.Update() # обновляем вид
...
def R_X_Y_obg(iSelectedObject): # получение координат и указатель на объект
    ...
    if Type_obj in (2, 3, 36): # Обработка объектов по типу объекта (2 - окружность; 3 - дуга; 36 - многоугольник, см. DrawingObjectTypeEnum)
        ...
        BD.append([R, X, Y, iSelectedObject, iView]) # добавляем запись координат в список (радиус/размер, положение X, положение Y, вид)
        ...
        BD.append([max(A, B), X, Y, iSelectedObject, iView]) # добавляем запись координат в список (радиус/размер, положение X, положение Y, вид)
...
def Centre(x, y, R, obj, iView): # простановка центров окружностей (радиус/размер, положение X, положение Y, вид)
...
def Point(x, y, obj, iView): # простановка точек в окружности (радиус/размер, положение X, положение Y, вид)
