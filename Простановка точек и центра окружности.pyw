...
Centre_repeat = False # повторять ли простановку центров уже выделеные обозначения центрров (True - да, False - нет)
...
Point_repeat = False # повторять ли простановку точек на уже выделеные точки (True - да, False - нет)
...
Conditional_sign_repeat = False # повторять ли простановку условные знаки на уже выделеные условные знаки (True - да, False - нет)
...
def R_X_Y_obg(iSelectedObject): # получение координат, указатель на объект и указатенль на вид
    ...
    if Type_obj == 2 and Circle and iSelectedObject.Style in iteration(Style): # обработка объектов по типу объекта и стилю (2 - окружность; 3 - дуга; 36 - многоугольник, см. DrawingObjectTypeEnum)
    ...
    elif Type_obj == 30 and Conditional_sign_var: # добавляем запись условных знаков
        ...
        if Point_on_center:
            list_Obg.append([0, X, Y, iSelectedObject, iView]) # добавляем запись координат в список
    ...
    elif Type_obj == 5 and Circle and Point_repeat == False: # обработка объектов по типу объекта (5 - точка, см. DrawingObjectTypeEnum)

        X = iSelectedObject.X # координата центра окружности по оси Х
        Y = iSelectedObject.Y # координата центра окружности по оси Х

        if Сenter_on_point:
            list_Obg.append([5, X, Y, iSelectedObject, iView]) # добавляем запись координат в список
...
def Conditional_sign(): # простановка условных знаков

    iFragment = iDocument2D.GetFragment() # получить указатель на интерфейс фрагмента

    fileName = "C:\Program Files\ASCON\KOMPAS-3D v19\Libs\Shapes_and_signs.kle|Условный знак 1_1.frw" # имя файла фрагмента (имя файла библиотеки|разделы, подразделы внутри библиотеки|имя фрагмента)
    iFragmentDefinition = iFragment.ksFragmentDefinition(fileName, None, 0) # определить фрагмент для вставки (указатель на фрагмент) (имя файла фрагмента, имя вставки, тип вставки (0 - взять в документ, 1 - внешней ссылкой))

    iPlacementParam = iKompasObject.GetParamStruct(7) # интерфейс параметров местоположения (привязки)
    iPlacementParam.angle = 10 # yгол поворота в системе координат вида
    iPlacementParam.scale_ = 1 # масштаб
    iPlacementParam.xBase = 10 # координаты базовой точки в системе координат вида
    iPlacementParam.yBase = 10 # координаты базовой точки в системе координат вида
